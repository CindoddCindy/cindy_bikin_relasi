...
import cindy.buat.Interface.AnggotaInterface;

@Controller("/anggota")
public class AnggotaController{

    private AnggotaInterface anggotaInterface;
    
    AnggotaController(AnggotaInterface anggotaInterface){
        this.anggotaInterface = anggotaInterface;
    }

    @Get(produces = MediaType.APPLICATION_JSON)
    public String index(@QueryValue int page, @QueryValue int limit) {
        List<Anggota> anggota = anggotaInterface.findAll(page, limit);
        HashMap<String, Object> data = new HashMap<>();
        data.put("page", Math.ceil(anggotaInterface.size() / limit));
    ...
    public String save(@Body Anggota anggota) {
        HashMap<String, Object> data = new HashMap<>();
        if (anggotaInterface.save(anggota)) {
            data.put("status", "ok");
        } else {
    ...
    public String destroy(@PathVariable Long id) {
        HashMap<String, Object> data = new HashMap<>();
        if (anggotaInterface.destroy(id)) {
            data.put("status", "ok");
        } else {
            data.put("status", "fail");
        }
